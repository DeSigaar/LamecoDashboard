...
router.post("/request", (req, res) => {
  ...
  User.findOne({
    ...
    // Find any open forgot requests and remove them
    ...
    Forgot.findOne({
      ...
      // If key is already taken, create a new one
      if (forgot) {
        for (let i = 0; i < 40; i++) {
          key += possible.charAt(Math.floor(Math.random() * possible.length));
        }

        // See if new key is unique - we'll try 1 more time
        Forgot.findOne({
          key
        }).then(forgot => {
          // If key is already taken, create a new one
    ...
    // Create a request and add to database
...
router.get("/info/:key", (req, res) => {
  // Get forgot request by given key
...
router.delete("/remove/:key", (req, res) => {
  ...
  Forgot.findOne({ key: req.params.key })
    .then(forgot => {
      // Remove forgot request from database
      forgot.remove().then(() => res.json({ success: true }));
    })
    .catch(err =>
      res
        .status(404)
        .json({ request_not_found: "Request not found with that key" })
    );
});
